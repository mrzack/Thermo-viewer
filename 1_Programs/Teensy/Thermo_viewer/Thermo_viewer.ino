...
The Thermo viewer is based on Lepton Flir, Teensy 3.1 and ILI9341 display.

Calibration code by Mark Ricklick was found from flir-lepton Google group.
...
//	Thermo scaler
	float temp = (float)min;
	float stepp = (float)(max-min) /200.0f;
	uint16_t scaler[200];
		for(x=0;x<200;x++){
			col = temp;
			col = col - min;
